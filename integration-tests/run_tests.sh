...
docker run -d --name tugbot-collect --link mock-result-service:mock-result-service -v /var/run/docker.sock:/var/run/docker.sock gaiadocker/tugbot-collect tugbot-collect --scanOnStartup --publishTestsTo http://mock-result-service:8085/json-results --publishTarGzTo http://mock-result-service:8085/tar-results
